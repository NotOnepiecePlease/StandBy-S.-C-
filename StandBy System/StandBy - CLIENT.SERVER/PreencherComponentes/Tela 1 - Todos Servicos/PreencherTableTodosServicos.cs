...
{
    class PreencherTableTodosServicos : conexao
    {
        MensagensErro mErro = new MensagensErro();
        //MensagensSucesso mSucesso = new MensagensSucesso();

        //Form 2 client server
        public void PreencherServicosAndamento(DataGridView tabelaServicosAndamento)
        {
            try
            {
                using(SqlConnection con = OpenConnection())
                {
                    ...
                    "sv_valorservico, sv_valorpeca, sv_lucro, sv_servico, sv_previsao_entrega, sv_existe_um_prazo, sv_form2_em_andamento, sv_form2_data_andamento from tb_servicos INNER JOIN tb_clientes on tb_servicos.sv_cl_idcliente = tb_clientes.cl_id WHERE sv_status = 1 and sv_ativo = 1 and sv_form2_em_andamento = 1", con);
                    ...
                    tabelaServicosAndamento.AutoGenerateColumns = false;
                    tabelaServicosAndamento.AllowUserToAddRows = false;
                    tabelaServicosAndamento.AllowUserToResizeColumns = true;
                    tabelaServicosAndamento.AllowUserToDeleteRows = false;
                    tabelaServicosAndamento.DataSource = datatable;
                    tabelaServicosAndamento.Sort(tabelaServicosAndamento.Columns["sv_form2_data_andamento"], ListSortDirection.Descending);
